...
from booking import Booking, BookingRequest as bookingreq
...
def get_all_free_bookings(db : db_dep):
    return db.get(models.Bookings).all()
...
def get_all_free_bookings_by_name(db: db_dep, restaurant_name: str = Query(min_length=2, max_length=100)):
    return db.get(models.Bookings).all().filter(models.Bookings.restaurant.casefold() == restaurant_name.casefold())

#POST example, not needed
#@router.post("/bookings/free", status_code=201)
#def add_free_booking(db: db_dep, booking: bookingreq):
    #TODO Needs to recieve or convert to date
#    new_booking = models.Bookings(**booking.model_dump())
#    db.add(new_booking)
#    db.commit()
...
#    booking_to_update = db.get(models.Bookings).all().filter(models.Bookings.id == id).first()
...
def get_booking_by_id(db: db_dep, id: int = Path(gt = 0)):
    matching_booking = db.get(models.Bookings).all().filter(models.Bookings.id == id).first()
